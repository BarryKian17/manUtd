...
@section('content')
      ...
      <div class="col-6 offset-3 mt-3">
        <div class=" card">
            <div class="rounded" >
              <form action="{{ route('admin#ticketCreate') }}" method="POST" novalidate="novalidate">
                  @csrf
                <h2 class="text-center text-dark pt-1">Ticket Create</h2><br>
                <div class="user-box mx-5 my-3">
                    ...
                    <select name="match" id="" class="form-control @error('match') is-invalid @enderror">
                        ...
                        @foreach ($match as $m)
                            <option value="{{ $m->id }}">{{ $m->home_team }} Vs {{$m->away_team}}   |   {{ $m->date }}</option>
                        @endforeach
                    </select>
                    @error('match')
